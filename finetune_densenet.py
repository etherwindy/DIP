...
def main(args):
    ...
    writer = SummaryWriter("output/densenet")

    model = DenseNet()
    backbone = torch.load("pretrainedModel/densenet/densenet.pth")
    features = model.backbone.classifier.in_features
    backbone.classifier = nn.Linear(features, 1)
